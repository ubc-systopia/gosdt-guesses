...
void GOSDT::fit(std::istream & data_source, std::unordered_set< Model > & models) {
    ...
    for (auto it = State::graph.bounds.begin(); it != State::graph.bounds.end(); it ++) {
        std::cout << "Tile is: " << it->first.to_string() << std::endl;
        vertex_accessor ver;
        State::graph.vertices.find(ver, it->first);
        Task task = ver->second;
        std::cout << "Associated Task has bound: [" << task.lowerbound() << ", " << task.upperbound() << "]" << std::endl;
        for (auto ele = it->second.begin(); ele != it->second.end(); ele ++) {
            std::cout << "Bound for feature " << std::get<0>(*ele) << " is [" << std::get<1>(*ele) << ", " << std::get<2>(*ele) << "]" << std::endl;
            child_accessor child;
            if(State::graph.children.find(child, std::make_pair(it->first, std::get<0>(*ele)))) {
                std::cout << "Associated positive Tile is: " << child->second.to_string() << std::endl;
            }
            if(State::graph.children.find(child, std::make_pair(it->first, -std::get<0>(*ele)))) {
                std::cout << "Associated negative Tile is: " << child->second.to_string() << std::endl;
            }
        }
        std::cout << std::endl;
    }
